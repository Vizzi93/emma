...
async def reset_admin_password():
    """Reset password for admin@emma.local to 'admin123'."""
    ...
    async with async_session() as session:
        # Find admin user
        query = select(User).where(User.email == 'admin@emma.local')
        result = await session.execute(query)
        user = result.scalar_one_or_none()

        if user is None:
            print('User admin@emma.local nicht gefunden!')
            return
